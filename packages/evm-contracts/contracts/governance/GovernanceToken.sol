...
contract GovernanceToken is ERC20, Ownable, ERC20Permit {
    ...
    // Transfer tokens (with delegation) using permit
    ...
    // Transfer tokens (with delegation)
    function transfer(
        address recipient,
        uint256 amount
    ) public virtual override returns (bool) {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "GT::delegateBySig: invalid signature"
        );
        require(
            nonce == _useNonce(signatory),
            "GT::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "GT::delegateBySig: signature expired"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GT (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "GT::_writeCheckpoint: block number exceeds 32 bits"
        );
