...
import { FluentCollector__factory, FluentCollector } from "../../typechain-types";
...
describe("FluentCollector", function () {
    ...
    let factory: FluentCollector__factory;
    let collector: FluentCollector;
    ...
    beforeEach(async function () {
        ...
        factory = await ethers.getContractFactory("FluentCollector", validator);
        collector = await upgrades.deployProxy(factory, [accountAddress, ADDR_RAND_2, ADDR_RAND], {
            kind: 'uups',
            redeployImplementation: 'always'
        }).then(x => x.connect(account)) as unknown as FluentCollector;
    });
